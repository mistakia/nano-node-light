...
class NanoPeer {
  constructor({ connectionInfo, nanoSocket }) {
    this.nodeId = null
...
export default class NanoNode extends EventEmitter {
  ...
  keepaliveLoop() {
    ...
    const selectedPeers = shuffled.slice(0, 8).map((p) => p.connectionInfo)
    const message = Buffer.concat(selectedPeers, 144)
  ...
  _getRepresentative({ account, readableAddress }) {
    const foundRep = this.representatives.get(account)
    if (foundRep) {
      return foundRep
    }

    log(`found new representative ${account}, at ${readableAddress}`)
  ...
  handleKeepalive = (body) => {
    for (let i = 0; i < 8; i++) {
      ...
      if (this.config.discover) this.connect(peer)
    }
  }

  onVote({ vote, readableAddress }) {
    ...
    if (vote.isValid) {
      ...
      log(`invalid vote received from ${readableAddress}, rep: ${repAddress}`)
    }
  }
  ...
  onTelemetry({ telemetry, connectionInfo, readableAddress }) {
    const result = {
      isPeer: true,
      isRejected: false,
      isLate: false,
    ...
    const peer = this.peers.get(connectionInfo.toString('binary'))
    ...
    if (!telemetryNodeId === peer.nanoSocket.connectedNodeId) {
      ...
      result.isPeer = false
      log(`mismatched telemetry & socket node_id (${readableAddress})`)
    }

    if (!peer.last_telemetry_req) {
      result.isRejected = true
      result.isUnsolicited = true
      log(`unsolicited telemetry_ack (${readableAddress})`)
    } else if (process.hrtime.bigint() - peer.last_telemetry_req > 1e10) {
      result.isRejected = true
      result.isLate = true
      log(`late telemetry_ack (${readableAddress})`)
    }
  ...
  setupNanoSocket = (nanoSocket) => {
    const { connectionInfo, readableAddress } = nanoSocket
    const peer = new NanoPeer({ connectionInfo, nanoSocket })

    nanoSocket.on('error', (error) => {
      this.emit('error', error, readableAddress)
    })
    ...
    nanoSocket.on('vote', (vote) => {
      this.emit('vote', vote)
      this.onVote({ vote, readableAddress })
    })
    ...
    nanoSocket.on('close', () => {
      this.peers.delete(connectionInfo.toString('binary'))
      log(
        `closed connection to ${readableAddress} (current: ${this.peers.size})`
      )
      nanoSocket.removeAllListeners()
      this.emit('close', connectionInfo)
    })

    this.peers.set(connectionInfo.toString('binary'), peer)
  }
}
