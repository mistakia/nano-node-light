...
import { PUBLIC_RPC_URLS, rpc } from 'nano-rpc'
...
export default class NanoNode extends EventEmitter {
  ...
  bootstrapWeights = async () => {
    const action = {
      action: 'representatives',
      count: 1000,
      sorting: true
    }
    const requests = PUBLIC_RPC_URLS.map((url) => rpc(action, { url }))
    const responses = await Promise.allSettled(requests)
    const weights = {}
    for (let i = 0; i < responses.length; i++) {
      const res = responses[i]
      if (res.value && !res.value.error) {
        for (const [repAddress, weight] of Object.entries(
          res.value.representatives
        )) {
          if (!weights[repAddress]) {
            weights[repAddress] = [weight]
          } else {
            weights[repAddress].push(weight)
          }
        }
      }
    }
