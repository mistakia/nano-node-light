...
export default class NanoSocket extends EventEmitter {
  constructor({ address, port, nodeKey, network }) {
    super()
    ...
    const stream = (this.stream = new NanoStream(this.network.ID))
    ...
    stream.on('message', (msg) => this.handleMessage(msg))
  ...
  sendMessage = ({ messageType, message, extensions }) => {
    ...
    this.socket.write(
      encodeMessage({ message, messageType, extensions, network: this.network })
    )
  }
  ...
  handleMessage = (msgInfo) => {
    ...
    log(
      `Received ${constants.MESSAGE_TYPE_NAME[message_type]} from ${this.address}:${this.port}`
    )
